...
# Ensure .sandbox/worktrees/ is in the target repo's .gitignore.
...
if [ -f "$GITIGNORE" ] && grep -qxF '/.sandbox/' "$GITIGNORE"; then
  # Migrate legacy broad ignore to narrow pattern
  sed -i 's|^/\.sandbox/$|/.sandbox/worktrees/|' "$GITIGNORE"
  printf '[install] narrowed /.sandbox/ → /.sandbox/worktrees/ in %s\n' "$GITIGNORE"
elif [ ! -f "$GITIGNORE" ] || ! grep -qxF '/.sandbox/worktrees/' "$GITIGNORE"; then
  printf '\n# Sandbox worktrees — generated by worktree-sandbox install.sh\n/.sandbox/worktrees/\n' >> "$GITIGNORE"
  printf '[install] added /.sandbox/worktrees/ to %s\n' "$GITIGNORE"
fi
